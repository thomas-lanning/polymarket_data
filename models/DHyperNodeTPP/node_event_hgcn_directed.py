...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='polymarket_fed_hourly',
                        help='Dataset name: polymarket_fed, polymarket_fed_hourly, polymarket_fed_daily, enron, etc.')
...
if __name__ == '__main__':
    ...
    if arg.use_wandb:
        import wandb
        wandb.init(project="Directed", entity="Submission", name= arg.fileID, group= dataset_name)

    Path("./nodeevent_directed/" + dataset_name + "/" + arg.fileID +"/").mkdir(parents=True, exist_ok=True)
    Path("./save_models/").mkdir(parents=True, exist_ok=True)
    save_path = "./nodeevent_directed/" + dataset_name + "/"  + arg.fileID +"/"

    # Create and configure logger
    logging.basicConfig(filename=save_path + dataset_name + '_' + arg.fileID + '_' +  '_' + str(arg.freq) + '_' + str(arg.seed) + '_' + type, format='%(asctime)s %(message)s', filemode='w')
    file_name =  save_path + arg.fileID + 'gcn ' + "Dataset {0}.txt mask {1} NegEdge {2}  EmbSize {3} type {4} seed {5}".format(
       dataset_name, arg.mask, arg.neg_e, arg.k, type, arg.seed)
    BEST_MODEL_PATH = f'./save_models/best_model_{dataset_name}_mask{arg.mask}_NegEdge{arg.neg_e}_EmbSize{arg.k}_freq{ arg.freq}_seed{arg.seed}_{arg.model_suffix}'
    print(file_name)
    ...
    for epoch in range(arg.start_epoch + 1, no_of_epochs):
        ...
        for i in iteration:
            ...
            if i > 0.1 * len(iteration) or dataset_name != 'eumail':
        ...
        if type == 'undirected':
            ...
            if dataset_name != 'enron' and  dataset_name != 'eumail' and dataset_name != 'twitter':
        ...
        if epoch % 10 == 0 or epoch == 99:
            ...
            # if (epoch % 10 == 0 or epoch ==99) and dataset_name != 'eumail' and  dataset_name != 'bitcoin':
